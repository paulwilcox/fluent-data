async function test () {
    ...
    try {
        ...
        if(!err.includes('outside the bounds of the matrix'))
            throw `Matrix.get([8,9], [7,8]) did not throw an out of bounds error.`;
    }
...
function compare(getType, matrixA, matrixB) {
    if(!matrixA.equals(matrixB, false))
        throw `Matrix.get(${getType}) has unexpected result.`;
}
